...
// --- FIX: ALLOW CUSTOM HEADERS FOR AUTH ---
app.use(cors({
    origin: '*', // Allow all origins (for now)
    methods: ['GET', 'POST', 'OPTIONS'],
    allowedHeaders: ['Content-Type', 'x-user-id'] // <--- CRITICAL: Allow our custom header
}));
...
let prices = { BTC: 0.00089, ETH: 0.32, DOGE: 0.0000869, SHIB: 0.000007, TON: 0.39, TRX: 0.08, LTC: 1.1, LUNA: 1.35, BC: 0.0001, USDT: 1 };
...
const getUser = (req) => {
    ...
    // Safety check: ensure user exists, otherwise fallback
...
app.post('/market-hack', (req, res) => { marketHackMultiplier = req.body.direction === 'up' ? 1.5 : 0.7; setTimeout(() => marketHackMultiplier = 1, 60000); res.json({ message: 'OK' }); });
