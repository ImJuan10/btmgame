...
function formatNumber(num, decimals = 2) { if (num == null || Number.isNaN(num)) return '—'; return Number(num).toLocaleString('en-US', { minimumFractionDigits: decimals, maximumFractionDigits: decimals }) }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-[#1e2329] border border-[#2b3139] rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-[#2b3139]"><h3 className="text-lg font-bold text-[#eaecef]">{title}</h3><button onClick={onClose} className="p-2 rounded-lg hover:bg-[#2b3139] text-[#848e9c]"><X size={20} /></button></div>
...
export default function WalletTab() {
  ...
  const refresh = async () => { try { const [h, p, b] = await Promise.all([getHoldings(), getPrices(), getBalances()]); setHoldings(h); setPrices(p); setBalanceHistory(Array.isArray(b) ? b : []) } catch (e) {} }
  useEffect(() => { refresh(); const interval = setInterval(refresh, 2000); return () => clearInterval(interval) }, [])

  const stats = useMemo(() => {
    if (balanceHistory.length < 2) return { current: 0, diff: 0, percent: 0, data: [], domain: [0, 100] }
    const now = Date.now(); const cutoff = CHART_RANGES_MS[portfolioChartRange] ? now - CHART_RANGES_MS[portfolioChartRange] : 0;
    ...
    const first = filtered[0].balance; const last = filtered[filtered.length - 1].balance;
    const diff = last - first; const percent = first !== 0 ? (diff / first) * 100 : 0;
    const pricesOnly = filtered.map(d => d.balance); const min = Math.min(...pricesOnly); const max = Math.max(...pricesOnly);
    const spread = max - min; const padding = spread * 0.2 || max * 0.05;
    return { current: last, diff, percent, data: filtered, domain: [min - padding, max + padding] }
  }, [balanceHistory, portfolioChartRange])

  const handleDeposit = async (e) => { e.preventDefault(); setLoading(true); const tid = toast.loading("Processing..."); try { await deposit(form.amount, form.currency); await refresh(); setModal(null); toast.success("Success", { id: tid }); setForm({ ...form, amount: '' }) } catch (err) { toast.error(err.message, { id: tid }) } finally { setLoading(false) } }
  const handleWithdraw = async (e) => { e.preventDefault(); setLoading(true); const tid = toast.loading("Processing..."); try { await withdraw(form.amount, form.currency, form.address); await refresh(); setModal(null); toast.success("Success", { id: tid }); setForm({ ...form, amount: '', address: '' }) } catch (err) { toast.error(err.message, { id: tid }) } finally { setLoading(false) } }
  const handleSwap = async (e) => { e.preventDefault(); setLoading(true); const tid = toast.loading("Swapping..."); try { await swap(form.currency, form.toCurrency, form.amount); await refresh(); setModal(null); toast.success("Success", { id: tid }); setForm({ ...form, amount: '' }); } catch (err) { toast.error(err.message, { id: tid }) } finally { setLoading(false) } }

  const isPositive = stats.diff >= 0
  const swapEstimate = form.amount && prices[form.currency] && prices[form.toCurrency] ? (parseFloat(form.amount) * prices[form.currency] / prices[form.toCurrency]) : 0;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 px-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#eaecef] flex items-center gap-2"><Wallet className="text-[#f3ba2f]" /> Wallet</h1>
        <div className="flex bg-[#1e2329] rounded-lg p-1 border border-[#2b3139]">{['1H', '1D', '1M', 'All'].map((r) => (<button key={r} onClick={() => setPortfolioChartRange(r)} className={`px-3 py-1 rounded-md text-xs font-bold transition-all ${portfolioChartRange === r ? 'bg-[#2b3139] text-[#eaecef]' : 'text-[#848e9c] hover:text-[#eaecef]'}`}>{r}</button>))}</div>
      </div>
          ...
          <form onSubmit={handleSwap} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
                <div className="relative" ref={swapFromRef}>
                    ...
                    <button type="button" onClick={() => setSwapFromOpen(!swapFromOpen)} className="w-full bg-[#0b0e11] border border-[#2b3139] rounded-xl p-3 text-[#eaecef] flex justify-between items-center text-sm font-bold hover:border-[#f3ba2f] transition-colors">{form.currency} <ChevronDown size={14} /></button>
                    {swapFromOpen && (<div className="absolute top-full left-0 w-full mt-2 bg-[#1e2329] border border-[#2b3139] rounded-xl shadow-xl z-50 max-h-48 overflow-y-auto">{COINS.map(c => (<div key={c} onClick={() => { setForm({...form, currency: c}); setSwapFromOpen(false)}} className={`px-4 py-2 text-sm font-bold cursor-pointer flex justify-between hover:bg-[#2b3139] ${form.currency === c ? 'text-[#f3ba2f]' : 'text-[#848e9c]'}`}>{c} {form.currency === c && <Check size={14}/>}</div>))}</div>)}
                </div>
                <div className="relative" ref={swapToRef}>
                    <label className="block text-xs font-black text-[#848e9c] uppercase mb-2">To</label>
                    <button type="button" onClick={() => setSwapToOpen(!swapToOpen)} className="w-full bg-[#0b0e11] border border-[#2b3139] rounded-xl p-3 text-[#eaecef] flex justify-between items-center text-sm font-bold hover:border-[#f3ba2f] transition-colors">{form.toCurrency} <ChevronDown size={14} /></button>
                    {swapToOpen && (<div className="absolute top-full left-0 w-full mt-2 bg-[#1e2329] border border-[#2b3139] rounded-xl shadow-xl z-50 max-h-48 overflow-y-auto">{COINS.filter(c => c !== form.currency).map(c => (<div key={c} onClick={() => { setForm({...form, toCurrency: c}); setSwapToOpen(false)}} className={`px-4 py-2 text-sm font-bold cursor-pointer flex justify-between hover:bg-[#2b3139] ${form.toCurrency === c ? 'text-[#f3ba2f]' : 'text-[#848e9c]'}`}>{c} {form.toCurrency === c && <Check size={14}/>}</div>))}</div>)}
                </div>
            </div>
            
            {/* FIX: Label OUTSIDE relative div, Button centered */}
            <div>
                <div className="flex justify-between mb-2">
                    <label className="block text-xs font-black text-[#848e9c] uppercase">Amount to Swap</label>
                    <span className="text-[10px] font-bold text-[#848e9c] cursor-pointer hover:text-[#eaecef]" onClick={() => setForm({...form, amount: swapBalance})}>Avail: {formatNumber(swapBalance, 6)}</span>
                </div>
                <div className="relative">
                    <input type="number" step="any" required value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} className="w-full bg-[#0b0e11] border border-[#2b3139] rounded-xl p-3 text-white font-mono focus:border-[#f3ba2f] outline-none" placeholder="0.00" />
                    <button type="button" onClick={() => setForm({...form, amount: swapBalance})} className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] bg-[#2b3139] px-2 py-1 rounded text-[#f3ba2f] hover:bg-[#363c45] font-bold transition-colors">MAX</button>
                </div>
            </div>
