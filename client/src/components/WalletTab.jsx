...
import { 
  ArrowDownToLine, ArrowUpFromLine, X, TrendingUp, ArrowUpRight, ArrowDownRight, Wallet, ArrowRightLeft, RefreshCw
} from 'lucide-react'
...
export default function WalletTab() {
  ...
  const [modal, setModal] = useState(null) // 'deposit', 'withdraw', 'swap'
  const [loading, setLoading] = useState(false)
  const [portfolioChartRange, setPortfolioChartRange] = useState('1H')
  
  // Unified form state
  const [form, setForm] = useState({ amount: '', currency: 'USDT', address: '', toCurrency: 'BTC' })
  ...
  const handleDeposit = async (e) => { e.preventDefault(); setLoading(true); const tid = toast.loading("Depositing..."); try { await deposit(form.amount, form.currency); await refresh(); setModal(null); toast.success("Success", { id: tid }); setForm({ ...form, amount: '' }) } catch (err) { toast.error(err.message, { id: tid }) } finally { setLoading(false) } }
  const handleWithdraw = async (e) => { e.preventDefault(); setLoading(true); const tid = toast.loading("Withdrawing..."); try { await withdraw(form.amount, form.currency, form.address); await refresh(); setModal(null); toast.success("Success", { id: tid }); setForm({ ...form, amount: '', address: '' }) } catch (err) { toast.error(err.message, { id: tid }) } finally { setLoading(false) } }
