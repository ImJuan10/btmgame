...
const TRADING_COINS = ['BTC', 'ETH', 'DOGE', 'SHIB', 'TON', 'TRX', 'LTC', 'LUNA', 'BC'].filter(
  (c) => c !== 'USDT'
)
...
const FILTERS = [
  { id: 'all', label: 'All', coins: TRADING_COINS },
  { id: 'majors', label: 'Majors', coins: MAJORS },
  { id: 'alts', label: 'Alts', coins: ALTS },
]
...
function ExpandedChart({ symbol, data, onClose, prices, holdings, refresh }) {
  ...
  const stats = useMemo(() => {
    ...
    const padding = spread * 0.4 || last * 0.05 

    const chartData = filtered.map((d) => ({
      ...d,
      value: d.price,
      label: formatChartLabel(d.timestamp, range),
    }))
  ...
  const handleBuy = async (e) => {
    ...
    setBuyLoading(true)
    try {
      await buy(pair, buyAmount)
      toast.success(`Bought ${buyAmount} ${symbol}`, { 
  ...
  const handleSell = async (e) => {
    ...
    setSellLoading(true)
    try {
      await sell(pair, sellAmount)
      toast.success(`Sold ${sellAmount} ${symbol}`, { 
        icon: '📉',
        style: { background: '#1e2329', color: '#f6465d', border: '1px solid #2b3139' }
      })
      setSellAmount('')
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              <div className="flex bg-[#0b0e11] rounded-lg p-1 border border-[#2b3139]">
                {Object.keys(CHART_RANGES_MS).map((r) => (
                  <button
                    key={r}
                    onClick={() => setRange(r)}
                    className={`px-3 py-1 rounded-md text-xs font-bold transition-all ${
                      range === r ? 'bg-[#2b3139] text-[#eaecef]' : 'text-[#848e9c] hover:text-[#eaecef]'
                    }`}
                  >
                    {r}
                  </button>
                  ...
                  >
                    <defs>
                      <linearGradient id={`grad-${symbol}`} x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={stats.isUp ? '#0ecb81' : '#f6465d'} stopOpacity={0.3} />
                        <stop offset="95%" stopColor={stats.isUp ? '#0ecb81' : '#f6465d'} stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    ...
                    <Tooltip
                      ...
                      content={({ active, payload }) => {
                        if (active && payload?.length) {
                          return (
                            <div className="bg-[#1e2329] border border-[#2b3139] p-3 rounded-xl shadow-2xl pointer-events-none">
                              <p className="text-[#848e9c] text-[10px] uppercase font-bold mb-1">
                                {new Date(payload[0].payload.timestamp).toLocaleString()}
                              </p>
                              <p className="font-bold text-[#eaecef] text-lg">
                                ${payload[0].value < 1 ? payload[0].value.toFixed(6) : formatNumber(payload[0].value)}
                              </p>
                            </div>
                          )
                        }
                    ...
                    <Area
                      type="monotone"
                      dataKey="value"
                      stroke={stats.isUp ? '#0ecb81' : '#f6465d'}
                      strokeWidth={3}
                      fill={`url(#grad-${symbol})`}
                      isAnimationActive={false}
                      activeDot={{ r: 6, fill: '#ffffff', stroke: stats.isUp ? '#0ecb81' : '#f6465d', strokeWidth: 2 }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
            ...
            <div className="bg-[#161a1e] rounded-2xl border border-[#2b3139] p-5 space-y-6 flex flex-col justify-center">
              <div>
                ...
                <form onSubmit={handleBuy} className="space-y-3">
                  <div className="flex justify-between text-[10px] font-bold uppercase text-[#848e9c]">
                    <span>Buy {symbol}</span>
                    <span>Avail: {formatNumber(holdings.wallet.USDT || 0)} USDT</span>
                  </div>
                  <div className="relative">
                    <input
                      ...
                      placeholder="Amount"
                      className="w-full bg-[#0b0e11] border border-[#2b3139] rounded-xl p-3 text-sm text-white focus:border-[#0ecb81] outline-none appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                    />
                    <span className="absolute right-3 top-3 text-[#848e9c] text-xs font-bold uppercase pointer-events-none">{symbol}</span>
                  </div>
                  <div className="text-[10px] text-[#848e9c] flex justify-between px-1">
                    <span>Est. Cost:</span>
                    <span className="text-[#eaecef] font-mono">
                      ≈ ${buyAmount ? formatNumber(parseFloat(buyAmount) * price) : '0.00'} USDT
                    </span>
                  </div>
              ...
              <div className="pt-4 border-t border-[#2b3139]">
                ...
                <form onSubmit={handleSell} className="space-y-3">
                  <div className="flex justify-between text-[10px] font-bold uppercase text-[#848e9c]">
                    ...
                    <span>Avail: {formatNumber(balance, 4)} {symbol}</span>
                  </div>
                  <div className="relative">
                    <input
                      type="number"
                      ...
                      placeholder="Amount"
                      className="w-full bg-[#0b0e11] border border-[#2b3139] rounded-xl p-3 text-sm text-white focus:border-[#f6465d] outline-none appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                    />
                    <span className="absolute right-3 top-3 text-[#848e9c] text-xs font-bold uppercase pointer-events-none">{symbol}</span>
                  </div>
                  <div className="text-[10px] text-[#848e9c] flex justify-between px-1">
                    <span>Est. Value:</span>
                    <span className="text-[#eaecef] font-mono">
                      ≈ ${sellAmount ? formatNumber(parseFloat(sellAmount) * price) : '0.00'} USDT
                    </span>
                  </div>
...
export default function MarketTab() {
  ...
  const refresh = async () => {
    try {
      const [p, h] = await Promise.all([getPrices(), getHoldings()])
      setPrices(p); setHoldings(h); setError(null)
    } catch (e) { setError("Server Connection Lost") }
  }

  useEffect(() => {
    refresh();
    const i1 = setInterval(refresh, 2000);
    const i2 = setInterval(async () => {
      for (const coin of TRADING_COINS) {
        getPricesHistory(`${coin}/USDT`).then(data => setHistory(h => ({ ...h, [coin]: data }))).catch(() => {})
      }
    }, 3000)
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${
                filter === f.id ? 'bg-[#2b3139] text-[#eaecef]' : 'text-[#848e9c] hover:text-[#eaecef]'
              }`}
            >
              {f.label}
            </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-xs text-[#848e9c] uppercase font-bold border-b border-[#2b3139]">
                ...
                <th className="py-4 px-6 text-right w-40">Market Trend</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#2b3139]/30">
              {displayCoins.map((symbol) => {
                ...
                const first = hist[0]?.price || price
                ...
                return (
                  <Fragment key={symbol}>
                    <tr
                      ...
                      className={`cursor-pointer transition-colors hover:bg-[#1e2329] ${expandedSymbol === symbol ? 'bg-[#1e2329]' : ''}`}
                    >
                      <td className="py-5 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-[#2b3139] flex items-center justify-center font-black text-xs">{symbol.substring(0, 2)}</div>
                          <span className="font-bold text-[#eaecef]">{symbol}/USDT</span>
                        </div>
                      </td>
                      <td className="py-5 px-6 text-right font-mono font-bold text-[#eaecef]">
                        {price < 1 ? price.toFixed(6) : formatNumber(price)}
                      </td>
                      <td className={`py-5 px-6 text-right font-bold ${isUp ? 'text-[#0ecb81]' : 'text-[#f6465d]'}`}>
                        {isUp ? '+' : ''}{pct.toFixed(2)}%
                      </td>
                      <td className="py-4 px-6">
                        <SparklineCell data={hist} symbol={symbol} />
                      </td>
                    </tr>
