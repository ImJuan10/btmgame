...
function SparklineCell({ data, symbol }) {
  ...
  if (chartData.length < 2) return <span className="text-[#848e9c] text-xs font-sans">—</span>
        ...
        <div className="p-6 border-t border-[#2b3139] animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              <span className="text-xl font-bold text-[#eaecef] font-sans">{symbol}/USDT</span>
            ...
            <div className="flex items-center gap-3">
              <div className="flex bg-[#0b0e11] rounded-lg p-1 border border-[#2b3139]">{Object.keys(CHART_RANGES_MS).map((r) => (<button key={r} onClick={() => setRange(r)} className={`px-3 py-1 rounded-md text-xs font-bold transition-all font-sans ${range === r ? 'bg-[#2b3139] text-[#eaecef]' : 'text-[#848e9c] hover:text-[#eaecef]'}`}>{r}</button>))}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div className="lg:col-span-3 h-[350px] bg-[#161a1e] rounded-2xl border border-[#2b3139] p-4 relative overflow-hidden">
              {stats.chartData.length >= 2 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={stats.chartData} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip isAnimationActive={false} cursor={{ stroke: '#848e9c', strokeWidth: 1, strokeDasharray: '4 4' }} content={({ active, payload }) => { if (active && payload?.length) { return (<div className="bg-[#1e2329] border border-[#2b3139] p-3 rounded-xl shadow-2xl pointer-events-none"><p className="text-[#848e9c] text-[10px] uppercase font-bold mb-1 font-sans">{new Date(payload[0].payload.timestamp).toLocaleString()}</p><p className="font-bold text-[#eaecef] text-lg font-mono">${payload[0].value < 1 ? payload[0].value.toFixed(6) : formatNumber(payload[0].value)}</p></div>) } return null }} />
              ...
              ) : ( <div className="h-full flex items-center justify-center text-[#848e9c] animate-pulse font-sans">Syncing market data...</div> )}
            </div>

            <div className="bg-[#161a1e] rounded-2xl border border-[#2b3139] p-5 space-y-6 flex flex-col justify-center">
              <div>
                <h3 className="text-xs font-black text-[#848e9c] uppercase tracking-widest mb-4 font-sans">Spot Trading</h3>
                {tradeError && <div className="text-xs text-[#f6465d] bg-[#f6465d]/10 p-2 rounded-lg mb-4 font-sans">{tradeError}</div>}
                
                <form onSubmit={handleBuy} className="space-y-3">
                  <div className="flex justify-between text-[10px] font-bold uppercase text-[#848e9c] font-sans"><span>Buy {symbol}</span><span className="font-mono">Avail: {formatNumber(usdtBalance)} USDT</span></div>
                  <div className="relative">
                    ...
                    {/* MAX BUTTON CENTERED */}
                    <button type="button" onClick={setMaxBuy} className="absolute right-12 top-1/2 -translate-y-1/2 text-[10px] bg-[#2b3139] px-2 py-1 rounded text-[#0ecb81] hover:bg-[#363c45] font-bold transition-colors font-sans">MAX</button>
                    {/* CURRENCY CENTERED */}
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[#848e9c] text-xs font-bold uppercase pointer-events-none font-sans">{symbol}</span>
                  </div>
                  <div className="text-[10px] text-[#848e9c] flex justify-between px-1 font-sans"><span>Est. Cost:</span><span className="text-[#eaecef] font-mono">≈ ${buyAmount ? formatNumber(parseFloat(buyAmount) * price) : '0.00'} USDT</span></div>
                  <button type="submit" disabled={buyLoading || !buyAmount} className="w-full py-3 bg-[#0ecb81] text-[#0b0e11] font-black rounded-xl uppercase hover:bg-[#0ecb81]/90 disabled:opacity-50 font-sans">Buy</button>
                </form>
              </div>

              <div className="pt-4 border-t border-[#2b3139]">
                <form onSubmit={handleSell} className="space-y-3">
                  <div className="flex justify-between text-[10px] font-bold uppercase text-[#848e9c] font-sans"><span>Sell {symbol}</span><span className="font-mono">Avail: {formatNumber(balance, 4)} {symbol}</span></div>
                  <div className="relative">
                    ...
                    {/* MAX BUTTON CENTERED */}
                    <button type="button" onClick={setMaxSell} className="absolute right-12 top-1/2 -translate-y-1/2 text-[10px] bg-[#2b3139] px-2 py-1 rounded text-[#f6465d] hover:bg-[#363c45] font-bold transition-colors font-sans">MAX</button>
                    {/* CURRENCY CENTERED */}
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[#848e9c] text-xs font-bold uppercase pointer-events-none font-sans">{symbol}</span>
                  </div>
                  <div className="text-[10px] text-[#848e9c] flex justify-between px-1 font-sans"><span>Est. Value:</span><span className="text-[#eaecef] font-mono">≈ ${sellAmount ? formatNumber(parseFloat(sellAmount) * price) : '0.00'} USDT</span></div>
                  <button type="submit" disabled={sellLoading || !sellAmount} className="w-full py-3 bg-[#f6465d] text-white font-black rounded-xl uppercase hover:bg-[#f6465d]/90 disabled:opacity-50 font-sans">Sell</button>
                </form>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 px-4 font-sans">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#eaecef] font-sans">Market Crypto</h1>
        <div className="flex bg-[#1e2329] rounded-lg p-1 border border-[#2b3139]">{FILTERS.map((f) => (<button key={f.id} onClick={() => setFilter(f.id)} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all font-sans ${filter === f.id ? 'bg-[#2b3139] text-[#eaecef]' : 'text-[#848e9c] hover:text-[#eaecef]'}`}>{f.label}</button>))}</div>
      </div>
